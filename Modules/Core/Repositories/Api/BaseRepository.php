...
class BaseRepository extends GeneralRepository implements BaseInterface
{
  ...
  public function getCreatedAttribute(Request $request)
  {
  ...
  public function store(Request $request)
  {
    ...
    return $this->created($this->__getModel()->create($this->getCreatedAttribute($request)), $request);
  }
...
//      $this->parseTranslatedAttributes($request), $this->sanitizeImages($request)
...
//        $request->validate(["$statusAttribute" => "required|boolean"]);
  ...
  public function insert(Request $request)
  {
    ...
    collect($request->data)->map(function ($item) use ($fillable, $translatedFillable, $locale) {

      foreach ($translatedFillable as $translated)
        $item[$translated] = json_encode([
          $locale => $item[$translated] ?? []
        ]);
